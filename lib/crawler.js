...
var Crawler = function(host,initialPath,initialPort,interval) {
	...
	// Search for the html title of the resource?
	crawler.parseHTMLTitle = false;
...
Crawler.prototype.handleResponse = function(queueItem,response,timeCommenced) {
	...
	function processReceivedData() {
		...
		// Get title out of the response
		if (crawler.parseHTMLTitle) {
			var data = responseBuffer.toString("utf-8");
			var match = data.match(/\<title\>(.+?)\<\/title\>/);
			if (null !== match) {
				stateData.title = match[1];
			}
		}
